...
func TestExpandFallback(t *testing.T) {
	unsetForTest(t, "KEMP_FALLBACK_TEST_UNSET")
	t.Setenv("KEMP_FALLBACK_TEST_SET", "real")
	t.Setenv("KEMP_FALLBACK_TEST_EMPTY", "")
	for _, tc := range []struct{ name, in, want string }{
		{"unset falls back", "${KEMP_FALLBACK_TEST_UNSET:-false}", "false"},
		{"set wins over default", "${KEMP_FALLBACK_TEST_SET:-false}", "real"},
		{"exported empty falls back", "${KEMP_FALLBACK_TEST_EMPTY:-other}", "other"},
		{"empty default allowed", "${KEMP_FALLBACK_TEST_UNSET:-}", ""},
		{"mixed with literal text", "a${KEMP_FALLBACK_TEST_UNSET:-b}c", "abc"},
	} {
	...
	if _, err := interpolate("${KEMP_FALLBACK_TEST_UNSET}"); err == nil {
...
func TestExpandSecretRejectsEmpty(t *testing.T) {
	t.Setenv("KEMP_EMPTY_SECRET_TEST", "")

	if _, err := interpolate("${KEMP_EMPTY_SECRET_TEST}"); err != nil {
		t.Fatalf("plain expansion must stay lenient on an exported-empty variable: %v", err)
	}
	if _, err := interpolateSecret("password", "${KEMP_EMPTY_SECRET_TEST}"); err == nil {
	...
	if got, err := interpolateSecret("password", "literal-pw"); err != nil || got != "literal-pw" {
		t.Errorf("literal credential: got %q err=%v", got, err)
	}
	if got, err := interpolateSecret("password", ""); err != nil || got != "" {
